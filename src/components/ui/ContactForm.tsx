...
type ContactMethod = "telegram" | "whatsapp" | "mail" | "phone" | "max";
...
export const ContactForm = () => {
    ...
    const MaxIcon = ({ className }: { className?: string }) => (
        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 720 720" fill="currentColor" stroke="none" className={className}>
    ...
    return (
        ...
        >
            ...
            <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                ...
                <div className="space-y-3 pt-2">
                    ...
                    <div className="flex gap-3 justify-between">
                        {[
                            ...
                            { id: "max" as ContactMethod, icon: MaxIcon },
                    ...
                    <AnimatePresence initial={false}>
                        {formData.contactMethods.includes("max") && (
                            <motion.div
                                initial={{ opacity: 0, y: -10 }}
                                animate={{ opacity: 1, y: 0 }}
                                exit={{ opacity: 0, y: -10 }}
                            ...
                            >
                                <div className="pt-3 pb-1">
                                    <input
                                        type="tel"
                                        placeholder="Номер телефона для Макс"
                                        className={inputStyles}
                                        value={formData.maxPhone}
                                        onChange={(e) => setFormData({ ...formData, maxPhone: e.target.value })}
                                        required
                                    />
                                </div>
                            </motion.div>
                ...
                <button
                    type="submit"
                    disabled={status === "loading" || !formData.consentData || !formData.name || !formData.phone || formData.contactMethods.length === 0}
                    className={`w-full font-medium py-4 rounded-xl transition-all duration-300 mt-4 shadow-lg
                        ${status === "loading" || !formData.consentData || !formData.name || !formData.phone || formData.contactMethods.length === 0
